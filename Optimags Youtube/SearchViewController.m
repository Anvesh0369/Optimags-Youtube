...
- (void)viewDidLoad {
    ...
    [_searchButton setImage:[IonIcons imageWithIcon:ion_ios_search iconColor:[UIColor redColor] iconSize:60.0f imageSize:CGSizeMake(10.0,10.0)] forState:UIControlStateNormal];
    
    //Youtube Service Object
    self.youtubeServices = [[GTLRYouTubeService alloc] init];
    //self.youtubeService.shouldFetchNextPages = true;
    self.youtubeServices.authorizer = _userObj.authentication.fetcherAuthorizer;
...
-(void)fetchVideoList:(NSString *)searchTerm
{
    ...
    [self.youtubeServices executeQuery:searchList delegate:self didFinishSelector:@selector(displayResultWithTicket:finishedWithObject:error:)];
